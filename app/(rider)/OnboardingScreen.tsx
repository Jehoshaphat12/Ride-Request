...
export default function RiderOnboardingScreen() {
  ...
  const uploadImage = async (uri: string, path: string): Promise<string> => {
    ...
    // Get public URL
    const { data } = supabase.storage.from("Ride Request User profiles").getPublicUrl(filePath);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (profilePic) {
        ...
        profilePicUrl = await uploadImage(profilePic, "profile-pictures");
        console.log("Profile picture uploaded:", profilePicUrl);
      } 

      console.log("Uploading license image:", licenseImage);
      licenseImageUrl = await uploadImage(licenseImage, "licenses");
